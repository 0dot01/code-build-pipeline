...
|----------|--------------|----------|
| Docker image missing | `docker images claude-worker` returns nothing | Suggest `docker build -t claude-worker ~/Projects/claude-pipeline/` |
| Container stuck (>20min) | `docker ps --filter name=pipeline-` shows long uptime | Kill and offer retry |
